...
export class ContentDrawer {
	...
	showKeyboard() {
		this.domCtrl.write(() => {
			...
			this.renderer.setElementStyle(this.element.nativeElement, 'top', this.thresholdBottom +'px');
		});
	};
	...
	hideKeyboard() {
		this.domCtrl.write(() => {
			this.renderer.setElementStyle(this.element.nativeElement, 'transition', 'top 0.5s');
			this.renderer.setElementStyle(this.element.nativeElement, 'top', this.thresholdTop + 'px');
		});
	}
